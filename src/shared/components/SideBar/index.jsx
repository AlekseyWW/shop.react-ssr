...
const BarItem = ({
	...
	return (
		<div>
			<Link to={url} key={category.id} className={styles}>
				{category.name}
			</Link>
			{sublist && (
				<div className={subStyles}>
					{sublist.map(item => {
						const itemStyles = classNames({
							...
							[`${style.SideBar__filter__sublist__item_active}`]:
								subCategoryId === item.slug,
						});

						const subPath = historyLocation
							? `${category.slug}/${item.slug}${historyLocation}`
			...
			{sublist.length && (
				<div className={subStyles}>
					{sublist.map(item => {
						...
						const parentCategory = find(categories, b =>
							find(b.items, { slug: item.slug })
						)
							? find(categories, b =>
									find(b.items, { slug: item.slug })
							  )
...
class SideBar extends PureComponent {
	...
	componentDidMount() {
		...
		if (
			this.props.brands.brands.length > 10 &&
			!openedBrands &&
			this.brandList
		) {
	...
	componentDidUpdate(prevProps) {
		if (
			this.props.brands.brands.length !==
				prevProps.brands.brands.length &&
			this.brandList
		) {
			if (this.brandList) {
				this.heightBrands = this.brandList.getBoundingClientRect().height;
	...
	historyPush = query => {
		...
		const pathname = slug
			? this.props.stockId
				? `/${this.props.stockId}/catalog/${slug}`
				: `/catalog/${slug}`
			: this.props.stockId
				? `/${this.props.stockId}/catalog`
				: `/catalog`;
	...
	render() {
		...
		const currentSizes =
			...
			sizes[0].sex &&
			sizes[0].sex.whom
				? _.filter(sizes, b => b.sex.name === sex)
				: sizes;
		const genderSize =
			sizes && sizes[0] && sizes[0].sex && sizes[0].sex.whom
				? _.groupBy(sizes, b => b.sex.whom)
				: [];
			...
			<div key="sidebar" className={style.SideBar}>
				<div className={style.SideBar__filter} ref={this.block}>
					<div className={style.SideBar__filter__item}>
							...
							<div className={style.SideBar__filter__item}>
								<BarFilter
									...
									onSubmit={data => {
										...
										query.sex = sex;
										query.offset =
											this.props.parsedQuery.offset || 0;
										query.count =
											this.props.parsedQuery.count || 12;
										this.historyPush(query);
										getProducts(
											query,
						...
						<div className={style.SideBar__filter__list}>
							{categories.map(category => (
								<BarItem
									...
									key={category.id}
									className={
										style.SideBar__filter__list__item
									}
						...
						<div className={style.SideBar__filter__list}>
							{renderedStockCategories &&
								renderedStockCategories.map(category => (
									<SubBarItem
										category={category}
										key={category.id}
										className={
							...
							<div className={style.SideBar__filter__list}>
								<div
									className={`${
										...
										style.SideBar__filter__list__item_active
									}`}
								>
									Пол
								</div>
								<div
									className={`${
										style.SideBar__filter__sublist
									} ${style.SideBar__filter__sublist_active}`}
								>
									{gender.map(gender => {
										const itemStyle = classNames({
											[`${
												style.SideBar__filter__sublist__item
											}`]: true,
											[`${
												style.SideBar__filter__sublist__item_active
											}`]:
												sex ===
												genderSize[gender][0].sex.name,
										});
										return (
											<div
												onClick={() =>
								...
								<div className={style.SideBar__filter__list}>
									...
									<div
										className={`${
											...
											style.SideBar__filter__sublist_active
										} ${style.SideBar__brands}`}
										ref={this.brandList}
									>
										{brands.brands.map((brand, id) => {
											const key = `brand-${id}`;
											var array = this.props.brand
												? this.props.brand.split(',')
												: [];
									...
									{brands.brands.length > 10 && (
										<div
											className={
												style.SideBar__brands__button
											}
											onClick={() => this.toggleBrands()}
										>
											{brandToogleText}
										</div>
									)}
								...
								<div className={style.SideBar__filter__list}>
									...
									<div
										className={`${
											...
											style.SideBar__filter__sublist_sizes
										}`}
									>
										{sortBy(
											uniqBy(currentSizes, 'name'),
											'name'
										).map((size, id) => {
											const key = `size-${id}`;
											var array = this.props.size
												? this.props.size.split(',')
												: [];
											var index = array.indexOf(
												size.name
											);
						...
						<Link
							...
							className={`${style.SideBar__filter__list__item} ${
								style.SideBar__filter__list__item_cart
							}`}
						>
							Перейти в&nbsp;корзину
						</Link>
