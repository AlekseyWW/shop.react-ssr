...
const PromoItem = ({
	...
	let url = slug
		? customCategory
			? `/${customCategory.slug}/catalog/${slug}`
			: `/catalog/${slug}`
		: customCategory
			? `/${customCategory.slug}/catalog`
			: `/catalog`;
		...
		<div className={style.PromoItem} style={styles}>
			<div className={style.PromoItem__inner}>
				<div className={style.PromoItem__content}>
					<p className={style.PromoItem__title}>
						{name ? HtmlParser(name) : HtmlParser(title)}
					</p>
					<p className={style.PromoItem__text}>
						{description
							? HtmlParser(description)
							: HtmlParser(
									'Для быстрого реагирования пишите нам в&nbsp;WhatsApp'
							  )}
						{!description && (
							<a
								href="https://api.whatsapp.com/send?phone=79286206404"
								target="_blank"
								className={style.PromoItem__icon}
							>
								<WhatsupIcon />
							</a>
						)}
					...
					{!description && (
						<p className={style.PromoItem__text}>
							{HtmlParser('Для просмотра каталога жмите ниже')}
						</p>
					)}
					{filter || customCategory ? (
						...
						<p className={style.PromoItem__button}>
							<NavLink
								to="/catalog"
								className={style.PromoItem__button__item}
								activeClassName={
