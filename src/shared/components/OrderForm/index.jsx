	...
	submit() {
		if (!this.props.valid) {
			this.alertify
				.okBtn('Закрыть')
				.alert(
					'Форма заполнена неверно! Пожалуста заполните все обязательные поля корректными данными.'
				);
		}
	}
	render() {
			...
			<form onSubmit={handleSubmit} className={style.OrderForm}>
				<div className={style.OrderForm__column}>
					<div className={style.OrderForm__label}>
						<span className={style.OrderForm__title}>
							Адрес доставки
						</span>
						<span className={style.OrderForm__note}>
				...
				<div className={style.OrderForm__column}>
					...
					<div className={style.OrderTable}>
						<div
							className={`${style.OrderTable__row} ${
								style.OrderTable__row_title
							}`}
						>
						...
						{products.map(product => (
							<div
								key={`${product.name}-${product.size.id}`}
								className={style.OrderTable__row}
							>
								<div
									className={`${style.OrderTable__cell} ${
										style.OrderTable__cell_name
									}`}
								>
					...
					<div className={style.OrderDeliver}>
						<div className={style.OrderDeliver__column}>
							...
							<p>
								При заказе на&nbsp;сумму
								от&nbsp;1&nbsp;500&nbsp;рублей, доставку почтой
								россии - БЕСПЛАТНО, и&nbsp;при заказе
								от&nbsp;4&nbsp;500 рублей&nbsp;&mdash; скидка
								500&nbsp;на доставку службой СДЕК.
							</p>
						</div>
						<div className={style.OrderDeliver__column}>
							{deliveryTypes &&
							...
							isDeliveryLoaded ? (
								filter(
									deliveryTypes,
									b => b.delivery !== 'electronic_payment'
								).map((type, id) => {
									const key = `item-${id}`;
									return type.code !== 'self_delivery' ? (
										<div
											key={key}
											className={style.OrderDeliver__item}
										>
											<Field
												name="deliveryType"
												component={CheckBox}
								...
								<p>
									После выбора города, здесь появятся
					...
					<div className={style.OrderPay}>
						...
						isDeliveryLoaded ? (
							...
							<p>
								После выбора города, здесь появятся доступные
								методы оплаты.
							</p>
						)}
					</div>
					<Button
						text="Оформить заказ"
						className={style.OrderPay__button}
						type="submit"
						onClick={() => this.submit()}
					/>
					<p>
						нажимая кнопку "Оформить заказ", Вы подтверждаете, что
						предоставляете свое согласие c{' '}
						<Link to="/agreement">
