...
const getCartSumm = added =>
	added.length ? added.reduce((summ, item) => summ + item.count, 0) : '';
	...
	return (
		<div className={style.Cart}>
			<Link to="/cart">
				...
				<span className={countStyle}>{getCartSumm(added) || 0}</span>
			</Link>
			{added.length > 0 && (
					...
					<table className={style.Cart__table}>
						<tbody>
							<tr className={style.Cart__table__row}>
								...
								<td
									className={`${style.Cart__table__cell} ${
										style.Cart__table__cell_name
									}`}
								>
									Модель
								</td>
							...
							{added.map((item, id) => {
								return (
									<tr
										key={`key-${id}`}
										className={style.Cart__table__row}
									>
										<td className={style.Cart__table__cell}>
											<img src={item.image} />
										</td>
										...
										<td className={style.Cart__table__cell}>
											<div
												className={
													style.Cart__table__cell__count
												}
											>
												<button
													onClick={() =>
