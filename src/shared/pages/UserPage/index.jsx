...
class UserPage extends Component {
	...
	render() {
		const { profile, favorites, orders } = this.props;
		const currentOrders =
			_.filter(orders, { status: 'formed' }).length > 0
				? _.filter(orders, { status: 'formed' })
				: orders;
		return (
						...
						profile.email && (
							<div className={styles.UserPage__header}>
								...
								<div className={styles.UserPage__contacts}>
									...
									<p>
										Работа отдела продажс 9:00 до 18:00
										вторник-воскресенье.
									</p>
						...
						<div className={styles.UserPage__container}>
							<div className={styles.UserPage__column}>
								<div className={styles.UserPage__item}>
									<div
										className={styles.UserPage__item__title}
									>
										<AdressIcon />
										Основной адрес доставки
									</div>
									<div
										className={styles.UserPage__item__text}
									>
										{profile.address ? (
											<div
												className={
													styles.UserPageList__list
												}
											>
												<div
													className={
														styles.UserPageList__delivery
													}
												>
													<p
														className={
															styles.UserPageList__note
														}
													>
														Имя
													</p>
											...
											<p>
												Тут будет отображаться ваш
												основной адрес
											</p>
										)}
										<Button to="/user/delivery">
											{profile.address
												? 'Изменить'
												: 'Добавить'}
										</Button>
									...
									>
										{currentOrders && currentOrders[0] ? (
											<Link to={`order/${orders[0].id}`}>
												<OrderProducts
													products={orders[0].colors}
													price={
														orders[0].deliveryPrice
													}
													sum={orders[0].sum}
												/>
											</Link>
								...
								<div className={styles.UserPage__item}>
									...
									>
										...
										в&nbsp;интернет-магазине необходимости
										в&nbsp;оформлении карты нет&nbsp;&mdash;
										сумма покупок сохраняется на&nbsp;вашем
										аккаунте (эл. почта). Достаточно
										совершать покупку с&nbsp;одного
										Логина/эл. почты. Подробнее
										о&nbsp;картах читайте в&nbsp;разделе
									</div>
								</div>
								<div className={styles.UserPage__item}>
									<div
									...
									<div
										className={styles.UserPage__item__text}
									>
										Добавляйте понравившиеся вещи
										в&nbsp;избранное, так они
										не&nbsp;потеряются
										и&nbsp;вы&nbsp;сможете приобрести
										их&nbsp;позже. Чтобы добавить вещь
										в&nbsp;избранное наведите на&nbsp;фото
										товара в&nbsp;каталоге и&nbsp;нажмите
										на&nbsp;символ сердца
									</div>
								</div>
							</div>
							<div className={styles.UserPage__column}>
								<div className={styles.UserPage__item}>
									...
									>
										...
										то&nbsp;укажите старый пароль
										и&nbsp;желаемый новый. После этого
										нажмите кнопку &laquo;Сохранить&raquo;
									</div>
								</div>
								<div className={styles.UserPage__item}>
									<RecoveryForm
							...
							<div className={styles.Page__container}>
								{favorites.added.map(product => (
									<ProductCard
										...
										actionText="убрать из избранного"
										sm
									/>
								))}
							</div>
