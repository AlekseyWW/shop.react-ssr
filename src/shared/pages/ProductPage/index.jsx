...
class ProductPage extends Component {
	...
	componentDidUpdate(prevProps, prevState) {
		if (
			...
			this.props.product.name !== prevProps.product.name
		) {
			this.props.getForProducts(`for${this.props.product.category.name}`);
		}
	}
	static fetchData({ store, params, query }) {
		...
		return store.dispatch(
			productsAction.getProductInfo(params.productId.split('?')[0], color)
		);
	}
	render() {
		const {
				...
				{isForLoaded &&
					...
					forProducts.length > 0 && (
						<NewPropducts
							...
							title="Так же вам может понравиться"
							mod="for"
						/>
					)}
			</div>
...
const mapDispatchToProps = (dispatch, ownProps) => {
	...
	return {
		getProductInfo: () =>
			dispatch(productsAction.getProductInfo(productId, { color })),
